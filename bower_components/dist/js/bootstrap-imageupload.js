...
(function($) {
    ...
    function getImageThumbnailHtml(src) {
        return '<img src="' + src + '" alt="Image preview" class="img-circle" style="display: block;margin-left: auto;margin-right: auto; width: ' + options.maxWidth + 'px; height: ' + options.maxHeight + 'px">';
    }
    ...
    function isValidImageFile(file, callback) {
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            callback(true, 'Image non valide.');
        }
        else {
            callback(false, 'Type de fichier non autorisé.');
        }
    }
    ...
    function isValidImageUrl(url, callback) {
        ...
        image.onload = function() {
            if (!timeout) {
                ...
                if ($.inArray(fileExtension, options.allowedFormats) > -1) {
                    callback(true, 'URL de l\'image non is valide.');
                }
                else {
                    callback(false, 'Type de fichier non autorisé.');
                }
        ...
        image.onerror = function() {
            if (!timeout) {
                window.clearTimeout(timer);
                callback(false, 'Image n\'a pas pu être trouvé.');
            }
        };
        ...
        timer = window.setTimeout(function() {
            ...
            callback(false, 'Chargement image timed out.');
        }, timeoutMs);
    }
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.btn span').text('Parcourir');
    ...
    function submitImageFile($fileTab) {
        ...
        $fileTab.find('.alert').remove();
        $fileTab.find('img').remove();
        $browseFileButton.find('span').text('Parcourir');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onerror = function() {
                    $fileTab.prepend(getAlertHtml('Erreur chargement de l\'image.'));
                    $fileInput.val('');
                };
